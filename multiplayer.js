...
// ============================================================
const MP = (function() {
    let WS_URL = 'wss://concern-electro-allen-nancy.trycloudflare.com'; // يمكن تغييره لاحقاً
    ...
    function handleMessage(msg) {
        switch (msg.type) {
            ...
            case 'swap_roles':
                // تبديل الأدوار بين المضيف والضيف (يستخدم في ألعاب "مُعد/مخمن")
                if (gameConfig && gameConfig.swapRoles) {
                    gameConfig.swapRoles();
                }
                break;
    ...
    // إرسال طلب تبديل الأدوار (يستخدمه المضيف أو أي طرف)
    function requestSwapRoles() {
        if (mode === 'remote') {
            send({ type: 'swap_roles' });
        }
    }
